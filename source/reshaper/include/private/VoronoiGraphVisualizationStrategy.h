...
public:
    ...
    LaTeXGraphWriter(const dpa::graph::utility::Point2D<int>& imageDims, std::ostream& output)
    ...
    */
    ...
    void initialize_vertex(Vertex vertex, const Graph& graph)
    {
        ...
        double x = graph[vertex].x;
        double y = graph[vertex].y;
...
private:
    ...
    const dpa::graph::utility::Point2D<int>& m_imageDims;

};

/*
    A class that writes LaTeX files to visualize
    similarity graphs
*/
template<typename Graph>
class VoronoiVisualizationStrategy: public graph::internal::ILaTeXVisualizationStrategy<Graph>
{
public:
    ...
    */
    void writeFile(const Graph& graph, const std::tuple<int, int>& imageDims,
        std::ostream& output) const noexcept override;
    ...
    /*
        ...
        @output A stream to write the graph output too
    */
    void writeHeader(const Graph& graph, std::ostream& output) const noexcept override;
    ...
    */
    ...
    /*
        ...
        @output A stream to write the graph output too
    */
    void writeTikzStyles(const Graph& graph, std::ostream& output) const noexcept override;

};

template<typename Graph>
void VoronoiVisualizationStrategy<Graph>::writeFile(const Graph& graph, const std::tuple<int, int>& imageDims,
...
template<typename Graph>
void VoronoiVisualizationStrategy<Graph>::writeHeader(const Graph& graph, std::ostream& output) const noexcept
{
...
template<typename Graph>
void VoronoiVisualizationStrategy<Graph>::writeDocument(const Graph& graph, const std::tuple<int, int>& imageDims,
                                                  std::ostream& output) const noexcept
{
...
template<typename Graph>
void VoronoiVisualizationStrategy<Graph>::writeTikzPicture(const Graph& graph, const std::tuple<int, int>& imageDims,
                                                     std::ostream& output) const noexcept
{
...
template<typename Graph>
void VoronoiVisualizationStrategy<Graph>::writeTikzStyles(const Graph& graph, std::ostream& output) const noexcept
{
    boost::ignore_unused_variable_warning(graph);

    output << "[";
    output << "node/.style={circle, draw=black, fill=red, thin, inner sep=0mm, minimum size=1mm}";
    output << "]";
}
}